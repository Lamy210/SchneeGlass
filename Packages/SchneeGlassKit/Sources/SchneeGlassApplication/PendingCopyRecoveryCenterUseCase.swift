...
/// Orchestrates Pending Copy Recovery without keeping security-scoped access alive across UI state.
///
/// Every listing assessment acquires a short-lived destination handle and releases it immediately.
/// Every mutation reloads the pending record and configuration, reacquires destination access, and
/// delegates to `PendingCopyRecoveryExecutionUseCase`, which performs another fresh assessment.
/// The UI therefore never owns a mutation-capable access handle or stale record authority.
