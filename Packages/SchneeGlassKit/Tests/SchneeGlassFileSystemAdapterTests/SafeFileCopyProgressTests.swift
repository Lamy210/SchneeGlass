...
private actor ProgressCopyEnvironment: CopyFileSystemAccessing, StagingCommitting {
    ...
    func copyItem(at sourceURL: URL, to stagingURL: URL) async throws {
        ...
        stagedIdentifiers[staging] = "progress-test:\(staging.lastPathComponent)"
    }
...
private actor ProgressPendingCopyStore: PendingCopyRecording {
...
private actor CopyProgressRecorder {
...
private func makeProgressCopyRequest(
    destination: URL,
...
@Test
func safeCopyReportsEachItemAsExecutionStarts() async throws {
    let destination = URL(fileURLWithPath: "/tmp/schneeglass-progress", isDirectory: true)
    let first = URL(fileURLWithPath: "/tmp/progress-first.txt")
    let second = URL(fileURLWithPath: "/tmp/progress-second.txt")
    let environment = ProgressCopyEnvironment(sources: [first: 10, second: 20])
    let engine = SafeFileCopyEngine(
        fileSystem: environment,
        committer: environment,
        recoveryStore: ProgressPendingCopyStore()
    )
    let request = try makeProgressCopyRequest(
    ...
    let recorder = CopyProgressRecorder()

    let result = await engine.copy(request) { progress in
    ...
    #expect(
        await recorder.snapshot() == [
            CopyProgress(
                currentIndex: 1,
                totalCount: 2,
                currentFilename: "first.txt"
            ),
            CopyProgress(
                currentIndex: 2,
                totalCount: 2,
                currentFilename: "second.txt"
            ),
        ]
    )
}
