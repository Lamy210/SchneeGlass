...
# A draft is mutable and can be cleaned safely if asset verification fails.
# Mark ownership before creation so cleanup can remove a partially-created tag.
CREATED_DRAFT=true
...
if [[ "$IS_IMMUTABLE" != 'true' ]]; then
  # Do not leave a mutable release public. Revert to draft, then cleanup removes it and the tag.
